...
onMounted(async () => {
  if (!chats.value || chats.value.length === 0) await chatsStore.fetch()
})
</script>
...
<template>
  <div class="container pt-5">
    ...
    <ul class="list-group" v-if="chats.length > 0">
      <li class="list-group-item" v-for="chat in chats" :key="chat.id">
        <RouterLink
          :to="{
            name: 'user-chat',
            params: { id: withoutCurrent(chat.participants)[0] || chat.participants[0] }
          }"
        ...
        >
          <div class="row">
            ...
            <div class="col">
              <span
                v-if="isUnread(chat)"
                class="badge ms-2 bg-primary p-2 rounded-circle float-end"
              >
              ...
              <small class="text-muted" v-if="chat.lastMessage">
                {{ getUser(chat.lastMessage.from)?.name || chat.lastMessage.from }}:
    ...
    <div v-else class="text-center mt-5">
      ...
      <p class="text-muted">
        <i class="bi bi-exclamation-circle-fill" /> Prueba a contactar con alguien en
