...
function onReport() {
  if (selected.value != '') {
    reportsStore
      .create(selected.value, text.value)
      .then(() => {
        toaster.success({
          text: 'Reportado con éxito. Gracias por hacer este lugar un poco más seguro.'
        })
        reportsStore.closeModal()
        selected.value = ''
        text.value = ''
        ...
        <div class="modal-body">
          <div class="mb-3">
            ...
            <select class="form-select" id="reportReason" v-model="selected" required>
              ...
              <option v-for="o in ReportReasons" :key="o.value" :value="o.value">
                {{ o.text }}
              </option>
            </select>
          </div>
